...
export default function AdminCashHistoryPage() {
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      ...
      if (cashError) {
        if (cashError.code === 'PGRST205') {
          console.warn('Tabela cash_sessions não existe.');
        } else {
          throw cashError;
        }
      }
      ...
      if (cashData) {
        const sessionPromises = cashData.map(async (session) => {
          let query = supabase
            .from('orders')
            .select('*')
            .in('status', ['delivered', 'completed']) // Assuming completed orders represent sales
    ...
    <div className="space-y-4 md:space-y-6 pb-20 lg:pb-0">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-8">
        ...
        <div className="lg:col-span-1 space-y-2 md:space-y-3">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-8 md:py-12 bg-white rounded-2xl border border-dashed border-gray-200">
              <Wallet className="w-6 h-6 text-gray-100 mb-2" />
            ...
            sessions.map((session) => (
              <motion.div
                layoutId={session.id}
                key={session.id}
                onClick={() => { setSelectedSession(session); setSelectedOrder(null); }}
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <p className="text-[8.5px] font-black text-gray-300 uppercase tracking-widest mb-0.5">ABERTURA</p>
                    <p className="font-black text-[var(--color-brand-dark)] text-[12px] md:text-sm">{formatDate(session.opened_at)}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-[8.5px] font-black text-gray-300 uppercase tracking-widest mb-0.5">FECHAMENTO</p>
                    {session.closed_at ? (
                      <p className="font-black text-[var(--color-brand-dark)] text-[12px] md:text-sm">{formatDate(session.closed_at)}</p>
                    ) : (
                      <span className="text-[9px] bg-green-50 text-green-600 px-1.5 py-0.5 rounded font-black uppercase tracking-wider border border-green-100">ABERTO</span>
                    )}
                  </div>
                </div>
            ...
            {selectedSession ? (
                  ...
                  <div className="flex justify-between items-start relative z-10">
                    <div>
                      <h2 className="text-xl font-black tracking-tight mb-1">
                        Aberto: {formatDate(selectedSession.opened_at)}
                      </h2>
                      {selectedSession.closed_at ? (
                        <h2 className="text-sm font-medium text-gray-300">
                          Fechado: {formatDate(selectedSession.closed_at)}
                        </h2>
                      ) : (
                        <span className="text-[10px] bg-green-500/20 text-green-400 px-2 py-0.5 rounded font-black uppercase tracking-wider border border-green-500/30">CAIXA ATUALMENTE ABERTO</span>
                      )}
                    </div>
                    <div className="text-right">
                      <p className="text-white/40 text-[9px] font-black uppercase tracking-widest mb-1">TOTAL DA SESSÃO</p>
                ...
                <div className="p-5">
                  <div className="space-y-2 max-h-[500px] overflow-y-auto pr-2 no-scrollbar">
                    {selectedSession.orders.length === 0 ? (
                      <p className="text-center text-sm font-bold text-gray-400 py-4">Nenhum pedido registrado nesta sessão.</p>
                    ) : (
                  ...
                  <div className="mt-5 pt-5 border-t border-dashed border-gray-200">
                    <div className="grid grid-cols-2 gap-3">
                      <div className="p-3 bg-[var(--color-brand-accent)]/5 rounded-xl border border-[var(--color-brand-accent)]/10">
                        ...
                        <p className="text-lg font-black text-[var(--color-brand-dark)]">R$ {(selectedSession.total / (selectedSession.count || 1)).toFixed(2).replace('.', ',')}</p>
                      </div>
                      <div className="p-3 bg-gray-50 rounded-xl border border-gray-100">
                        <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Volume da Sessão</p>
                        <p className="text-lg font-black text-[var(--color-brand-dark)]">{selectedSession.count} Pedidos</p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="h-[60vh] flex flex-col items-center justify-center bg-gray-50/50 rounded-[2.5rem] border-2 border-dashed border-gray-200 p-8 text-center">
                <div className="p-6 rounded-3xl bg-white shadow-sm mb-6">
                  <Wallet className="w-12 h-12 text-[var(--color-brand-accent)] opacity-20" />
                </div>
                <h3 className="font-black text-[var(--color-brand-dark)] text-xl mb-2">Selecione uma Sessão</h3>
                <p className="text-sm text-gray-400 max-w-xs mx-auto font-medium">Clique em uma sessão à esquerda para visualizar o faturamento detalhado e os pedidos realizados no período.</p>
              </div>
            )}
          </AnimatePresence>
        </div>
      ...
      <AnimatePresence>
        {selectedSession && (
          ...
          >
            <motion.div 
              key={`mobile-modal-premium-${selectedSession.id}`}
              initial={{ y: "100%" }}
              animate={{ y: 0 }}
              exit={{ y: "100%" }}
            ...
            >
                  ...
                  <div>
                    ...
                    {selectedSession.closed_at ? (
                      <p className="text-[11px] font-medium text-gray-300 mt-0.5">
                        Fch: {formatDate(selectedSession.closed_at)}
                      </p>
                    ) : (
                       <span className="text-[8px] bg-green-500/20 text-green-400 px-1.5 py-0.5 rounded font-black uppercase tracking-wider border border-green-500/30 mt-1 inline-block">ABERTO</span>
                    )}
                  </div>
                  <div className="text-right">
                    <p className="text-white/40 text-[8.5px] font-black uppercase tracking-widest mb-0.5">TOTAL</p>
                    <p className="text-lg font-black text-[#ff914a]">R$ {selectedSession.total.toFixed(2).replace('.', ',')}</p>
                  </div>
                </div>
              ...
              <div className="flex-1 overflow-y-auto no-scrollbar bg-gray-50/50">
                <div className="p-3 space-y-1.5 pb-24">
                  {selectedSession.orders.length === 0 ? (
                    <p className="text-center text-[11px] font-bold text-gray-400 py-4">Nenhum pedido nesta sessão.</p>
                  ) : (
                    selectedSession.orders.map((order) => (
                      <motion.div 
      ...
      <AnimatePresence>
        {selectedOrder && (
          <div 
            className="fixed inset-0 z-[110] flex items-end lg:items-center justify-center lg:p-4 bg-[#381010]/40 backdrop-blur-md"
